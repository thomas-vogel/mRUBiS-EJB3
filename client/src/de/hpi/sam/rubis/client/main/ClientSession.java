...
public class ClientSession {
	...
	private void initialize() {
		...
		try {
			...
			this.browseRegionsService = (BrowseRegionsService) ctx
					.lookup(BrowseRegionsService.MAPPED_NAME + "#" + BrowseRegionsService.class.getCanonicalName());

			this.userRegistrationService = (UserRegistrationService) ctx.lookup(
					UserRegistrationService.MAPPED_NAME + "#" + UserRegistrationService.class.getCanonicalName());

			this.aboutMeService = (AboutMeService) ctx
					.lookup(AboutMeService.MAPPED_NAME + "#" + AboutMeService.class.getCanonicalName());

			this.browseCategoriesService = (BrowseCategoriesService) ctx.lookup(
					BrowseCategoriesService.MAPPED_NAME + "#" + BrowseCategoriesService.class.getCanonicalName());

			this.itemRegistrationService = (ItemRegistrationService) ctx.lookup(
					ItemRegistrationService.MAPPED_NAME + "#" + ItemRegistrationService.class.getCanonicalName());

			this.bidService = (BidService) ctx
					.lookup(BidService.MAPPED_NAME + "#" + BidService.class.getCanonicalName());

			this.viewUserInfoService = (ViewUserInfoService) ctx
					.lookup(ViewUserInfoService.MAPPED_NAME + "#" + ViewUserInfoService.class.getCanonicalName());

			this.buyNowService = (BuyNowService) ctx
					.lookup(BuyNowService.MAPPED_NAME + "#" + BuyNowService.class.getCanonicalName());

			this.reputationService = (ReputationService) ctx
					.lookup(ReputationService.MAPPED_NAME + "#" + ReputationService.class.getCanonicalName());
	...
	public void initiliazeSellerSide() {
		try {
			...
			User seller = this.userRegistrationService.registerUser("Ryan", "Terry", "ryan", "Ryan.Terry@mail.com",
					"PAE20CZC2TL", regionName, CustomerClass.GOLD);
			System.out.println(seller.infoString());
			UserProfile sellerUserProfile = this.aboutMeService.getAboutMe(seller.getNickname(), seller.getPassword());
			System.out.println(sellerUserProfile.infoString());

			System.out.println("Item to be sold:");
			List<Category> categories = this.browseCategoriesService.getAllCategories();
			assert categories.size() > 0;
			Category category = null;
			for (Category c : categories) {
				if (c.getName().equals("USA")) {
					category = c;
					break;
				}
			}
			...
			Item item = this.itemRegistrationService.registerItem("The National - Trouble Will Find Me",
					"Trouble Will Find Me is the highly anticipated sixth album of The National.", 1, 9.99f, 12.99f,
					11.50f, new Date(System.currentTimeMillis()),
					new Date(System.currentTimeMillis() + (1000 * 60 * 60 * 2)), seller.getId(), seller.getPassword(),
					category.getId());
	...
	public void initializeBuyerSide() {
		try {
			...
			User buyer = this.userRegistrationService.registerUser("Emma", "Bailey", "emma", "Emma.Bailey@mail.com",
					"QFC45DAW7NC", regionName, CustomerClass.SILVER);
			System.out.println(buyer.infoString());
			UserProfile buyerUserProfile = this.aboutMeService.getAboutMe(buyer.getNickname(), buyer.getPassword());
	...
	public void bidOrBuyItem() {
		try {
			List<Item> items = this.browseCategoriesService.getItemsByName(this.itemName);
			Item itemToBuy = null;
			for (Item i : items) {
				if (i.getName().equals(this.itemName) && i.getId() == this.itemId) {
					itemToBuy = i;
					break;
				}
			}
			...
			int sellerId = itemToBuy.getSeller().getId();
			System.out.println("SellerId : " + sellerId);

			UserInfo remoteSeller = this.viewUserInfoService.getUserInfo(sellerId);
			System.out.println(remoteSeller.infoString());

			List<Bid> bidsOnItemToBuy = this.bidService.getItemBidHistorySortyByBidPrice(itemToBuy.getId());
			...
			Bid bid = this.bidService.bidOnItem(itemToBuy.getId(), currentMaxPrice + 1f, currentMaxPrice + 2f, 1,
			...
			bidsOnItemToBuy = this.bidService.getItemBidHistorySortyByBidPrice(itemToBuy.getId());
			for (Bid b : bidsOnItemToBuy) {
				System.out.println("    - " + b.infoString());
			}

			// buy item
			BuyNow buyNow = this.buyNowService.buyItemNow(itemToBuy.getId(), 1, this.buyerNickname, this.buyerPassword);
			System.out.println("bought item ... : " + buyNow.infoString());

			// add comment
			Comment comment = this.reputationService.giveComment(this.buyerNickname, this.buyerPassword, sellerId,
	...
	/**
	 * USES THE HUGE mRUBiS DATA SET!
	 */
	public void searchItems() {
		try {
			List<Item> items = this.browseCategoriesService.getPersonalizedItems("IsFrederick344", "pjnvqcydkzmwkydg");
			System.out.println(items.size());
			items = this.browseCategoriesService.getPersonalizedItems("GlMendez91", "secretly");
			System.out.println(items.size());
			items = this.browseCategoriesService.getPersonalizedItems("ShHolland108", "ypeeetvtzsuiabyx");
			System.out.println(items.size());
		} catch (BrowseCategoriesServiceException e) {
			e.printStackTrace();
		}
	...
	/**
	 * USES THE SMALL mRUBiS DATA SET!
	 */
	public void searchItems1() {
		try {
			List<Item> items = this.browseCategoriesService.getPersonalizedItems("ethan", "OKK69ZXQ7MI");
			System.out.println(items.size());
		} catch (BrowseCategoriesServiceException e) {
			e.printStackTrace();
		}
	...
	public void userMgmtTest() {
		try {
			UserInfo info = this.viewUserInfoService.getUserInfo(1);
		...
		try {
			UserProfile profile = this.aboutMeService.getAboutMe("ethan", "OKK69ZXQ7MI");
	...
	public void buyNowTest() {
		try {
			int number = this.buyNowService.getNumberOfAvailableInstances(2);
