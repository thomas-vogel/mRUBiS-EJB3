...
/**
 * Implementation of an {@link ItemFilter} that filters items based on their
 * categories, that is, all items of a certain category are filtered.
 * 
 * @author thomas vogel
 * 
 */
@Stateless(mappedName = ItemFilter.CATEGORY_ITEM_FILTER_MAPPED_NAME)
public class CategoryItemFilterBean extends AbstractFilterBean implements
		ItemFilter {

	/**
	 * The next filter to invoke.
	 */
	@EJB(mappedName = ItemFilter.RECOMMENDATION_ITEM_FILTER_MAPPED_NAME)
	...
	private static ItemFilterMonitor monitor = new ItemFilterMonitor(
			CategoryItemFilterBean.class.getCanonicalName());
	...
	/**
	 * Comma separated list of category names. Items of these categories are
	 * filtered.
	 */
	@Resource
	...
	public List<Item> filter(List<Item> items, User user)
		...
		// lookup filtered categories =====================================
		try {
			InitialContext ctx = new InitialContext();
			this.filteredCategories = (String) ctx
					.lookup("java:comp/env/filteredCategories");
		} catch (NamingException e) {
			e.printStackTrace();
		}
		// end of lookup filtered categories =================================

		String[] categoryNames = this.filteredCategories.split(",");
	...
	@PostConstruct
	public void init() {
		super.setFilterId(CategoryItemFilterBean.class.getCanonicalName());
